// app/(tabs)/(collector)/authcheck/selectnft.jsx
...
export default function SelectNFTPage() {
  ...
  const handleSelectItem = (item) => {
    router.back();
    router.setParams({ selectedItem: encodeURIComponent(JSON.stringify(item)) });
  };
  ...
  const handleUpload = () => {
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        ...
        (i) => {
          if (i === 1) pickFromGallery();
          if (i === 2) pickFromFiles();
        }
      );
    } else {
  ...
  const handleSource = (id) => {
    if (id === 'rack')        router.push('/authcheck/fromrack');
    if (id === 'marketplace') router.push('/authcheck/frommarketplace');
    if (id === 'scan')        Alert.alert('Camera', 'Use a real device to scan QR.');
    if (id === 'upload')      handleUpload();
  };

  return (
    <View style={styles.container}>
      {/* ✅ No SafeAreaView — tab layout handles safe area */}
