// app/(tabs)/item/[itemId]/index.jsx
...
const STATUS_CONFIG = {
  Requested: { color: '#888',    icon: 'time-outline',             label: 'Requested' },
  Prepared:  { color: '#2980B9', icon: 'cube-outline',             label: 'Prepared'  },
  Shipped:   { color: '#E67E22', icon: 'car-outline',              label: 'Shipped'   },
...
const QRSection = ({ item }) => {
  ...
  const handleDownload = async () => {
    try {
      ...
      // ✅ Step 1 — Fetch QR as PNG from qrserver and convert to base64
      ...
      // ✅ Step 2 — Build HTML with embedded QR image
      const html = `
        <!DOCTYPE html>
        <html>
          <head>
...
export default function ItemDetailPage() {
  ...
  const item = MOCK_ITEMS[decodeURIComponent(itemId)] ?? MOCK_ITEMS['#AZEDR'];
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.body}>
          ...
          {/* QR Code — Claimed only */}
          {status === 'Claimed' && <QRSection item={item} />}

          <View style={{ height: 100 }} />
        </View>
      </ScrollView>

      {/* Claim button — Shipped only */}
      {status === 'Shipped' && (
        <TouchableOpacity
