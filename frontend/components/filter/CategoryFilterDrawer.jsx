// components/CategoryFilterDrawer.jsx
...
const ALL_LABELS = CATEGORIES.map((c) => c.label);

export default function CategoryFilterDrawer({ visible, onClose, onApply, selected }) {
  ...
  const toggleSelect = (label) => {
    setLocalSelected((prev) => {
      ...
      // ✅ If all individual items are selected, treat as "all selected"
      const allChecked = ALL_LABELS.every((l) => next.includes(l));
      return allChecked ? ALL_LABELS : next;
    });
  };

  // ✅ Select all = check every category
  const selectAll = () => setLocalSelected([...ALL_LABELS]);

  // ✅ All categories checked = all labels are in localSelected or empty = none selected
  const isAllSelected = ALL_LABELS.every((l) => localSelected.includes(l)) || localSelected.length === 0;
  const isSelected    = (label) => localSelected.includes(label) || localSelected.length === 0;

  const handleApply = () => {
    // ✅ If all selected or none selected, pass empty array = no filter
      ...
      <View style={styles.root}>
        ...
        >
          ...
          <ScrollView style={styles.drawerScroll} showsVerticalScrollIndicator={false}>
            ...
            {/* ✅ All Categories — checks everything */}
            <TouchableOpacity
            ...
            {CATEGORIES.map((cat) => (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <View style={[styles.checkbox, isSelected(cat.label) && styles.checkboxActive]}>
                  {isSelected(cat.label) && <Ionicons name="checkmark" size={14} color="#fff" />}
                </View>
